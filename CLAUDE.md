...
**Vier Regeln gelten in jeder gebauten Oberfläche.** *Farbe wird nach ihrer Aufgabe benannt, nicht nach ihrem Aussehen.* Komponenten kennen Rollen wie `handlung` oder `fehler`, niemals Farbwerte; die Palette ist die einzige Datei mit einem Hexadezimalwert darin, siehe [`02_BRANDING/02-visuelles-system.md`](02_BRANDING/02-visuelles-system.md). *Zugänglichkeit wird geprüft, nicht beansprucht.* Kontrast wird aus den Rollen gerechnet, jede ausgelieferte Seite läuft durch eine Prüfung, und die Rechtslage bleibt eine Frage an den Kunden statt an uns, siehe [`07_ENGINEERING/04-accessibility-und-animation.md`](07_ENGINEERING/04-accessibility-und-animation.md). *Was nach dem ersten Bild eintrifft, hat vier Zustände* und nicht zwei, und ein Skelett gehört nur dorthin, wo wirklich etwas lädt, siehe [`04_UI/08-wartezustaende-und-skelette.md`](04_UI/08-wartezustaende-und-skelette.md). *Jedes Element trägt die Beweislast für sich selbst:* Was kann ein Besucher nicht mehr, wenn es weg ist? „Es wirkt leerer" ist keine Antwort, siehe [`04_UI/04-cards-content-surfaces.md`](04_UI/04-cards-content-surfaces.md). Dicht ist dabei nicht überladen; eine Preistabelle darf voll sein.
