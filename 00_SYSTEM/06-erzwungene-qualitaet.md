...
| Fließende Wurzel-Schrift verlangt Pixel-Breakpoints | 3 | Wächst die Wurzel-Schriftgröße mit der Viewport-Breite, skaliert das rem-System auf großen Schirmen hoch. rem-Breakpoints werden in genau dieser wachsenden Schrift gemessen und verschieben sich mit ihr, lösen also auf großen Breiten zu spät aus. Sobald die Wurzel fluid ist, sind die Breakpoints in px definiert und die berechneten Werte wachsen über eine Breitenleiter monoton, ohne dass unter dem Skalierungsbeginn ein Wert vom alten abweicht. Warum in [Der große Bildschirm ist nicht die feste Fassung](../04_UI/08-grosser-bildschirm-und-skalierung.md). |
| Layouttragendes Maß in rem, nicht in bare px | 3 | In einem hochskalierenden rem-System steht ein festes Pixelmaß still, während alles um es herum wächst, und wirkt relativ zu klein. Höhen und Breiten tragender Flächen werden gegen bare Pixelwerte geprüft; eine bewusste Ausnahme (randlose Fläche, feste Kunst-Richtung) wird benannt statt stillschweigend gelassen. |
| Inhaltsbild ohne stille Rückfalllücke | 3 | AVIF fällt bei einem Dekodier-Fehler NICHT auf WebP/JPG zurück: Der `<picture>`-Rückfall greift nur bei fehlender Formatunterstützung, nicht bei einer unterstützten, aber nicht darstellbaren Datei. Ein Browser, der AVIF meldet, es aber nicht rendert, zeigt ein leeres Bild ohne Rückfall, und der Build im Headless-Browser sieht das nie. Inhaltsbilder liefern deshalb WebP mit JPG-Rückfall; AVIF nur mit erklärter, geprüfter Browsermatrix. Warum in [Der Build ist nicht der Browser des Gastes](../07_ENGINEERING/06-der-build-ist-nicht-der-browser-des-gastes.md). |
| Eigene Schrift ohne metrisch angepasste Fallback-Schrift | 3 | Ohne einen per size-adjust, ascent-override und descent-override an die Zielschrift angeglichenen Fallback springt das Layout beim Schriftwechsel und kostet CLS genau im ersten Eindruck. Zu jeder selbst gehosteten Schrift existiert eine metrisch angepasste Fallback-Schrift. |
