...
| Genau ein Auftritt je Betrieb | 3 | Ein Altauftritt in einem Unterverzeichnis stört niemanden, der ihn kontrollieren könnte: Der Inhaber gibt die Domain ein und landet auf der neuen Seite, die Agentur prüft die neue, und die Suchmaschine behält den alten Bestand, weil sie weder `404` noch `301` noch `noindex` bekommt. Gefunden wurde in einem Projekt ein Auftritt von 2017 unter `/html/`, mit falschen Öffnungszeiten, ohne Impressumsseite, ohne `viewport` und ohne eigenen Einstieg — die alten Zeiten waren bereits in ein Branchenportal gewandert. Ein Skript ruft die üblichen Verzeichnisnamen ab und vergleicht die indexierten Adressen mit der Sitemap; was indexiert ist und in keiner Sitemap steht, meldet es. Ausgearbeitet in [Der Altauftritt](../06_SEO/07-der-altauftritt-den-niemand-abgeschaltet-hat.md). |
